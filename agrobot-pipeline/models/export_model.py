...
class DeepStreamOutput(nn.Module):
    ...
    def forward(self, x):
        x = x.transpose(1, 2)                          # [batch, 8400, num_classes+4]
        boxes = x[:, :, :4]                            # [batch, 8400, 4]
        class_scores = x[:, :, 4:]                     # [batch, 8400, num_classes]
        labels = torch.argmax(class_scores, dim=-1, keepdim=True).float()  # [batch, 8400, 1]
        scores = torch.gather(class_scores, 2, torch.argmax(class_scores, dim=-1, keepdim=True).long())  # [batch, 8400, 1]
        return torch.cat([boxes, scores, labels], dim=-1)  # [batch, 8400, 6]
