...
class Tutorial4:
    def __init__(self):
        ...
        while not self.terminate:
            msg = bus.timed_pop_filtered(100 * Gst.MSECOND,   # timeout of 100 milliseconds (desired timeout must be specified in nanoseconds)
        ...
        if msg.type == Gst.MessageType.ERROR:
            ...
            if msg.src == self.playbin:
                ...
                if playing:
                    ...
                    if self.playbin.query(query):  # result stored in same query
                        f, seek_enabled, start, end = query.parse_seeking()  # parse_seeking() extracts a boolean, and range in which seeking is possible
                        if seek_enabled:
